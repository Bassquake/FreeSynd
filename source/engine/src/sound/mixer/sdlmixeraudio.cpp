...
bool SdlMixerAudio::openFile(std::string path) {
    // Lock the audio device so the callback cannot be mid-execution while we
    // reinitialise the player's internal state via adl_openFile.
    SDL_LockAudio();
    is_playing = 0;
    int result = adl_openFile(pMidiPlayer, path.c_str());
    SDL_UnlockAudio();
...
void SdlMixerAudio::playMusic(int songNb, bool loop) {
    // Lock the audio device so the callback cannot race against adl_selectSongNum.
    SDL_LockAudio();
    is_playing = 0;
