...
void SystemSDL::initialize(bool fullscreen) {
    ...
    #ifdef __ANDROID__
        // Block the render thread on pause so eglSwapBuffers is never called on a destroyed surface
...
bool SystemSDL::pumpEvents(FS_Event &evtOut) {
    ...
    if (SDL_PollEvent(&evtIn)) {
        ...
        case SDL_MOUSEBUTTONUP:
            ...
            if (evtIn.button.which == SDL_TOUCH_MOUSEID && rightClickMode_) {
                evtOut.button.button = SDL_BUTTON_RIGHT;
                rightClickMode_ = false;
            } else {
                evtOut.button.button = evtIn.button.button;
                if (evtIn.button.which == SDL_TOUCH_MOUSEID) {
                    lastTouchUpTime_ = SDL_GetTicks();
                    lastTouchUpWinX_ = evtIn.button.x;
                    lastTouchUpWinY_ = evtIn.button.y;
                }
            }
        ...
        case SDL_MOUSEBUTTONDOWN:
            {
            if (evtIn.button.which == SDL_TOUCH_MOUSEID && lastTouchUpWinX_ >= 0) {
                int dx = evtIn.button.x - lastTouchUpWinX_;
                int dy = evtIn.button.y - lastTouchUpWinY_;
                if ((SDL_GetTicks() - lastTouchUpTime_) < 300u && (dx*dx + dy*dy) < 100*100) {
            ...
            evtOut.button.x = p.x;
            evtOut.button.y = cursor_y_ = p.y;
            evtOut.button.button = (evtIn.button.which == SDL_TOUCH_MOUSEID && rightClickMode_)
                ? SDL_BUTTON_RIGHT : evtIn.button.button;
            evtOut.button.keyMods = keyModState_;
            }
            break;
